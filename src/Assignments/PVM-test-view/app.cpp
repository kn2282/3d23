...
//    glm::vec2 scale{0.5, 0.5};
    ...
    glm::mat4 proj = glm::perspective<GLfloat >(glm::pi<GLfloat>()/2.0f, 1.0f, -1.0f, 1.0f);
    std::cout<<glm::to_string(proj)<<std::endl;
    glm::mat4 view = glm::lookAt<GLfloat>(glm::vec3(-0.25f, 0.25f, -2.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, 0 * sizeof(GLfloat), 32 * sizeof(GLfloat), &PVM);
...
//    glBufferSubData(GL_UNIFORM_BUFFER, 8 * sizeof(GLfloat), 2 * sizeof(GLfloat), &rot[1]);
    ...
    glGenBuffers(9, &vec_buffer_unit);
